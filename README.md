...
python3 launch.py
```
> 如果你在项目根目录（包含 `Get_transform` 文件夹的上一层目录）执行命令，请改用 `python Get_transform/launch.py` 或 `python3 Get_transform/launch.py`。
启动脚本会自动检查Python版本、安装缺失的依赖，然后启动主程序。

**或者直接运行主程序**
```在Windows PowerShell中运行以下命令（从 Get_transform 目录）
python duplicate_file_cleaner.py
```
```在macOS/Linux终端中运行以下命令（从 Get_transform 目录）
...
- ✅ Python 版本检测（最低 3.6）
- ✅ pip 可用性检测
- ✅ 自动创建/读取 `Get_transform/requirements.txt`
